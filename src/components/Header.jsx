import React from "react";
import { Link } from "react-router-dom";
import { UseWishList } from "../redux/WishListSlice";
import { UseCartSlice } from "../redux/CartSlice";

const Header = ({ toggleShopCategory }) => {
  const { wishListKeyboards, wishListSwitches } = UseWishList()
  const {KeyboardsList,SwitchesList} =UseCartSlice()
  return (
    <div className="h-[100px] flex items-center justify-between">
      <Link to={"/"} className="flex items-center gap-2 text-xl font-semibold">
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
          strokeWidth={1.5}
          stroke="currentColor"
          className="w-8 h-8"
        >
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            d="M12 9.75L14.25 12m0 0l2.25 2.25M14.25 12l2.25-2.25M14.25 12L12 14.25m-2.58 4.92l-6.375-6.375a1.125 1.125 0 010-1.59L9.42 4.83c.211-.211.498-.33.796-.33H19.5a2.25 2.25 0 012.25 2.25v10.5a2.25 2.25 0 01-2.25 2.25h-9.284c-.298 0-.585-.119-.796-.33z"
          />
        </svg>
        Space
      </Link>
      <div className="flex items-center gap-10">
        <button className="flex items-center gap-1 text-xl font-medium" onClick={toggleShopCategory}>
          Shop
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth={1.5}
            stroke="currentColor"
            className="w-6 h-6 text-yellow-600"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z"
            />
          </svg>
        </button>
        <Link to={'keyboards'} className="font-medium text-xl">Keyboards</Link>
        <Link to={'switches'} className="font-medium text-xl">Switches</Link>
        <Link to={'Sound'} className="font-medium text-xl flex items-center gap-1">Sound
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6 text-yellow-600">
            <path strokeLinecap="round" strokeLinejoin="round" d="M9 9l10.5-3m0 6.553v3.75a2.25 2.25 0 01-1.632 2.163l-1.32.377a1.803 1.803 0 11-.99-3.467l2.31-.66a2.25 2.25 0 001.632-2.163zm0 0V2.25L9 5.25v10.303m0 0v3.75a2.25 2.25 0 01-1.632 2.163l-1.32.377a1.803 1.803 0 01-.99-3.467l2.31-.66A2.25 2.25 0 009 15.553z" />
          </svg>

        </Link>
      </div>
      {/* navigation */}
      <div className="flex items-center gap-5">
        <div className="w-[250px] relative">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth={1.5}
            stroke="currentColor"
            className="w-6 h-6 absolute left-2 top-2/4 -translate-y-2/4 text-[#aaa]"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z"
            />
          </svg>
          <input
            type="text" placeholder="Search..."
            className="outline-none border border-[#eee] rounded-md w-full py-2 pl-10 pr-2"
          />
        </div>
          <Link to={'Wishlist'} className="w-10 h-10 rounded-full bg-[#eee] relative flex items-center justify-center cursor-pointer">
            {
              (wishListKeyboards.length > 0 || wishListSwitches.length > 0) && <span className="w-3 h-3 rounded-full bg-red-500 absolute right-0 top-0"></span>
            }
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              strokeWidth={1.5}
              stroke="currentColor"
              className="w-5 h-5"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
                d="M21 8.25c0-2.485-2.099-4.5-4.688-4.5-1.935 0-3.597 1.126-4.312 2.733-.715-1.607-2.377-2.733-4.313-2.733C5.1 3.75 3 5.765 3 8.25c0 7.22 9 12 9 12s9-4.78 9-12z"
              />
            </svg>
          </Link>
          <Link to={'Cart'} className="w-10 h-10 relative rounded-full bg-[#eee] flex items-center justify-center cursor-pointer">
        {
              (KeyboardsList.length > 0 || SwitchesList.length > 0) && <span className="w-3 h-3 rounded-full bg-red-500 absolute right-0 top-0"></span>
            }
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
              <path strokeLinecap="round" strokeLinejoin="round" d="M2.25 3h1.386c.51 0 .955.343 1.087.835l.383 1.437M7.5 14.25a3 3 0 00-3 3h15.75m-12.75-3h11.218c1.121-2.3 2.1-4.684 2.924-7.138a60.114 60.114 0 00-16.536-1.84M7.5 14.25L5.106 5.272M6 20.25a.75.75 0 11-1.5 0 .75.75 0 011.5 0zm12.75 0a.75.75 0 11-1.5 0 .75.75 0 011.5 0z" />
            </svg>
          </Link>
        <div className="w-10 h-10 rounded-full bg-[#eee] flex items-center justify-center cursor-pointer">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
            <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 6a3.75 3.75 0 11-7.5 0 3.75 3.75 0 017.5 0zM4.501 20.118a7.5 7.5 0 0114.998 0A17.933 17.933 0 0112 21.75c-2.676 0-5.216-.584-7.499-1.632z" />
          </svg>
        </div>
      </div>
      {/* navigation */}
    </div>
  );
};

export default Header;
